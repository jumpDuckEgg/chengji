$(function(){
        ...
        $(".initials ul li").click(function(){
            ...
            if(_index==0){
                $('#c-area').animate({scrollTop: '0px'}, 300);//点击第一个滚到顶部
            }else if(_index==27){
                var DefaultTop=$('#default').position()? $('#default').position().top:0 ;
                $('#c-area').animate({scrollTop: DefaultTop+'px'}, 300);//点击最后一个滚到#号
            }else{
                var letter = _this.text();
                if($('#'+letter).length>0){
                    var LetterTop = $('#'+letter).position().top;
                    $('#c-area').animate({scrollTop: LetterTop+'px'}, 300);
                }
            }
        })
