...
$(function () {
    initBaidu();
    $('.headbar-right').click(function(){
        if($(this).text()=='更多'){
            $('.orderTicket-box__content').hide();
            $('.orderTicket-box__operation').show();
            $(this).text('完成');
            return false;
        }
        if($(this).text()=='完成'){
            $('.orderTicket-box__content').show();
            $('.orderTicket-box__operation').hide();
            $(this).text('更多');
            return false;
        }
    });
    $('.orderInfo-box__info-item-place').click(function () {
        $("#stationDetail").show();
    })
    // 隐藏详情
    ...
    // 全程路线
    $('.orderInfo-box__info-otherbox-item-line').click(function () {
